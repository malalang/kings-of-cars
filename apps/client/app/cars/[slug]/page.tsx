...
import { ArrowLeft, Gauge, Fuel, Settings2, ShieldCheck, MapPin, CheckCircle2 } from 'lucide-react'
...
export default async function VehiclePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const gallery = Array.isArray(car.gallery_urls) && car.gallery_urls.length ? car.gallery_urls : (car.image_url ? [car.image_url] : [])
  ...
  return <main className="koc-shell">
    <section style={{background:'#111',color:'#fff',padding:'28px 0 64px'}}>
      <div className="koc-container">
        <Link href="/cars" style={{display:'inline-flex',alignItems:'center',gap:8,color:'rgba(255,255,255,.65)',fontSize:11,fontWeight:800,textTransform:'uppercase',letterSpacing:'.12em'}}><ArrowLeft size={14}/> Back to showroom</Link>
    ...
    <section style={{padding:'54px 0 70px'}}>
      <div className="koc-container">
        ...
        <section style={{marginTop:64}}><div className="koc-kicker">Vehicle gallery</div><h2 className="koc-display" style={{fontSize:'clamp(34px,4vw,54px)',marginTop:8}}>Explore every angle</h2><p style={{color:'var(--koc-muted)',marginTop:12}}>Click the main image to open the full-screen gallery. Use the thumbnails or arrow keys to move between available photos.</p><div style={{marginTop:24}}><VehicleGallery images={gallery} alt={title}/></div></section>

        <div style={{display:'grid',gridTemplateColumns:'minmax(0,1.1fr) minmax(280px,.9fr)',gap:50,marginTop:70}}>
          ...
          <aside className="koc-card" style={{padding:28,alignSelf:'start'}}><ShieldCheck color="var(--koc-red)"/><h3 style={{fontSize:22,fontWeight:900,marginTop:16}}>Vehicle confidence</h3><p style={{color:'var(--koc-muted)',lineHeight:1.7,marginTop:10}}>Vehicle information is maintained from the dealership inventory source. Confirm final specification, availability and pricing with King of Cars before purchase.</p><Link href={`/contact?vehicle=${car.id}`} className="koc-button koc-button-primary" style={{marginTop:22,width:'100%',justifyContent:'center'}}>Enquire about this car</Link></aside>
        </div>
      </div>
    </section>
  </main>
