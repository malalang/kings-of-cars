...
function SocialStrip() {
  return <div className="koc-legacy-social" aria-label="Share vehicle">
    <span className="share-plus">+</span><span className="share-whatsapp">W</span><span className="share-facebook">f</span><span className="share-mail"><Mail size={12} /></span>
  </div>
}
...
export default async function CarsPage({ searchParams }: { searchParams: Promise<CarSearchParams> }) {
  ...
  const transmissions = [...new Set(vehicles.map((car) => car.transmission).filter(Boolean))].sort()
  const fuels = [...new Set(vehicles.map((car) => car.fuel_type).filter(Boolean))].sort()
  ...
  return <>
    <main className="koc-legacy-cars-page">
      <div className="koc-legacy-inner">
        <div className="koc-legacy-title title left"><h1>Trichardts Road, Boksburg Used Cars</h1><div className="divider" /></div>
