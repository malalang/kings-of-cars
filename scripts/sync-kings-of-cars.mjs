...
import {
  ...
  mapVehicle,
  PAGE_SIZE,
} from "./lib/kingofcars-engine-api.mjs";
...
const UPSERT_BATCH_SIZE = Number(
  process.env.KINGS_OF_CARS_UPSERT_BATCH_SIZE ?? 50,
);
...
async function rebuildImages(rowsBySlug) {
  ...
  for (const vehicle of rowsBySlug) {
    const gallery = Array.isArray(vehicle.gallery_urls)
...
async function main() {
  ...
  console.log(
    `Engine API returned ${sourceRows.length} vehicles; finalCount=${finalCount}`,
  );
  console.log(`Request payload: ${JSON.stringify(payload)}`);

  if (!finalCount || finalCount < MIN_EXPECTED || finalCount > MAX_EXPECTED) {
    throw new Error(
      `Unexpected source inventory count ${finalCount}; expected ${MIN_EXPECTED}-${MAX_EXPECTED}. Refusing to modify production inventory.`,
    );
  }

  const vehicles = sourceRows
    .map(mapVehicle)
    .filter((vehicle) => vehicle.slug && vehicle.model);
  const unique = [
    ...new Map(vehicles.map((vehicle) => [vehicle.slug, vehicle])).values(),
  ];
  console.log(`Mapped ${unique.length} unique vehicles.`);

  if (
    unique.length < Math.min(MIN_EXPECTED, Math.floor(finalCount * 0.9)) ||
    unique.length > MAX_EXPECTED
  ) {
    throw new Error(
      `Mapped ${unique.length} vehicles from expected source count ${finalCount}. Refusing to modify production inventory.`,
    );
  }
  ...
  for (let index = 0; index < unique.length; index += UPSERT_BATCH_SIZE) {
    ...
    console.log(
      `Upserted ${Math.min(index + batch.length, unique.length)}/${unique.length}`,
    );
  }

  const importedBySlug = new Map(importedRows.map((row) => [row.slug, row.id]));
  const galleryRows = unique
    .map((vehicle) => ({
      ...vehicle,
  ...
  console.log(
    `VERIFIED: table rows=${finalTableCount}; available=${availableCount}; source=${finalCount}`,
  );
  if (
    (availableCount ?? 0) < Math.min(MIN_EXPECTED, Math.floor(finalCount * 0.9))
  ) {
    throw new Error(
      `Verification failed: only ${availableCount} available vehicles after sync.`,
    );
  }
